...
if __name__ == '__main__':
    ...
    if target_domain in nli_domains:
        so = GenPromptCL.SentenceOrthogonality(768, 256, device, num_classes=3).to(device)
    else:
        so = GenPromptCL.SentenceOrthogonality(768, 256, device).to(device)
    ...
    cov_weighting_loss = GenPromptCL.CoVWeightingLoss(mean_sort, mean_decay_param, device, save_losses=args.save_losses,
                                             target_domain=args.target_domain)
    ...
    for epoch in range(num_epochs):
        ...
        for step, batches in enumerate(pbar):
            ...
            # logits_list: Store the logits of the MASK
            # masked_hidden_states_list: Store the hidden states of the MASK
            # sentence_hidden_states_list: Store the hidden states of the CLS
            ...
            for idx, batch in enumerate(batches):
                ...
                # logits: (batch_size, num_outputs), logits of MASK
                # masked_hidden_states: (batch_size, hidden_size), hidden states of MASK
                # cls_hidden_states: (batch_size, hidden_size), hidden states of CLS
            ...
            if args.md_adv:
                ...
                try:
                    ...
                    # Solve the problem of inconsistent data sizes in a batch
