...
def load_amazon_dataset(dir_path, target_domain, tokenizer, max_length):
    ...
    unsup_dataset = UnsupCustomDataset(unsup_train_texts, tokenizer, max_length)
...
def load_rumour_dataset(dir_path, target_domain, tokenizer, max_length):
    train_texts, train_labels, train_datasets, unsup_train_texts = [], [], [], []
...
def load_mnli_datasets(dir_path, target_domain, tokenizer, single_sentence_max_length, max_length):
    ...
    unsup_train_dataset = UnsupCustomDataset(unsup_train_texts, tokenizer, max_length)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target_domain',
                        ...
                        default='book')
    parser.add_argument('--pooling', choices=['cls', 'pooler', 'last-avg', 'first-last-avg'],
                        default='cls')
    ...
    if target_domain in sa_domains:
        if target_domain == 'sst' or target_domain == 'imdb':
            train_datasets, unsup_train_dataset = load_amazon_dataset(amazon_dir_path, target_domain, tokenizer,
                                                                      max_length)
    ...
    model = SimCSE.SimCSEModel(model_path, args.pooling, device).to(device)
    classifier = SimCSE.Classifier(hidden_size=768, output_size=num_outputs).to(device)
    ...
    for epoch in range(unsup_epochs):
        for batch in tqdm.tqdm(unsup_train_dataloader, desc='unsupervised training'):
            ...
            y_pred = model(input_ids, att_masks)
            loss = model.simcse_unsup_loss(y_pred)
    ...
    if not os.path.exists('results/scores/simcse/'):
        os.makedirs('results/scores/simcse/')

    with open(f'results/scores/simcse/{target_domain}.txt', 'a') as f:
